...
class AlgarvuRing {
    public static void main(String[] args){
        ...
        algarvuRing(i);
        System.out.println("\n"+"=".repeat(51)+ ".");
        System.out.println("xXBl4ck_LigHtningzXx                "+ new java.sql.Timestamp(System.currentTimeMillis()));
  }//main
    ...
    public static void algarvuRing(int a){
        ...
        for (int i = a-1; (loendur < 5) && (i > 10); i--) {
            if (i%2!=0 && i%3 != 0 && i%5 != 0 && i%7 != 0 && kasSobib(i)) {
                boolean lipp = true;
                double c = Math.pow(10.0, (double)String.valueOf(i).length()-1);
     ...
     */
    static int viimaneEsimeseks(int h, double a){
        int uus = h/10;
        int algus = h%10;
        return (int)a*algus+uus;
    }//viimaneEsimeseks
    ...
    /**
     * Algarvu sisalduvuse kontrolli meetod, sisuliselt sama, mis kasSobib, aga kuna oleme arvus 0, 2, 4, 5, 6, 8 sisalduvust vaadanud pole see enam vajalik
     *
     * Meetod tagastab false kui tegu ei ole algarvuga.
     * leitud ja kohandatud: https://stackoverflow.com/questions/2385909/what-would-be-the-fastest-method-to-test-for-primality-in-java
     */
    static boolean kasSobiks(int n) {
        int sqrtN = (int)Math.sqrt(n)+1;
        for(int i = 6; i <= sqrtN; i += 6) {
            if(n%(i-1) == 0 || n%(i+1) == 0) return false;
        }
